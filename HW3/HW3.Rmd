...
```{r echo=FALSE}
mod <- stan(data = stan_df, file = here("C:/GitHub/STA2201/STA2201-W2023/HW3/fertility_pop.stan"), 
            chains = nchain1, iter = iter1, warmup =1000, refresh = 0)
tab3 <- summary(mod)[["summary"]][c(paste0("beta[",1:3, "]"), paste0("alpha_age[",1:5, "]"), paste0("alpha_edu[",1:5, "]")),]
tab3
```
```{r echo = FALSE}
alpha_age <- extract(mod)[["alpha_age"]] |> as.data.frame() |> 
...
```{r echo=FALSE}
beta <- tab3[1:3, c(1, 5, 8)] |> as.data.frame()
alpha_age <- tab3[4:8, c(1, 5, 8)] |> as.data.frame()
alpha_edu <- tab3[9:13, c(1, 5, 8)] |> as.data.frame()
```
...
Overall the Bayesian model yields the least accurate estimates of the proportion of women who want kids in a given age group and the true proportions, while the post-stratified estimates are the most accurate, except for the group of women aged 35-39. The $p_{raw}$ estimates seem unstable.
