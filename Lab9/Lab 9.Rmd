...
```{r}
observe.i <- c(5,13,18,5,10,18,29,10,15,22,4,11,10,22,13,14,17,21,25,6,11,21,13,5,19,18,14,17,3,10,
...
expect.i <- c(6.17,8.44,7.23,5.62,4.18,29.35,11.79,12.35,7.28,9.40,3.77,3.41,8.70,9.57,8.18,4.35,
...
## Question 1: Explain a bit more what the `expect.i` variable is. For example, if a particular area has an expected deaths of 6, what does this mean?

Expected deaths is the implied number of lip cancer deaths for a particular region given that region's age structure and the national level age-specific mortality rates for lip cancer. For example, an expected number of deaths of 6 would mean that particular region, we would expected 6 lip cancer deaths if this region were to experience the same age specific mortality rates as at the national level.

## Question 2: Run three different models in Stan with three different set-up's for estimating $\theta_i$, that is the relative risk of lip cancer in each region:
...
**Model 1:** $\log \theta_i = \alpha + \beta x_i$

```{r echo=FALSE}
stan_df1 <- list(y = observe.i, log_e = log(expect.i), N = length(observe.i), x = aff.i - mean(aff.i))
mod1 <- stan(data = stan_df1, file = here("C:/Users/pquyn/OneDrive/Personal Documents/GitHub/STA2201-W2023/Lab9/lab9_1.stan"), chain = 3, iter = 500, refresh = 0)
summary(mod1)[["summary"]][c("alpha", "beta"),]
```

**Model 2:** $\log \theta_i = \alpha_i + \beta x_i$

```{r echo=FALSE}
stan_df2 <- list(y = observe.i, log_e = log(expect.i), N = length(observe.i), x = aff.i - mean(aff.i))
mod2 <- stan(data = stan_df1, file = here("C:/Users/pquyn/OneDrive/Personal Documents/GitHub/STA2201-W2023/Lab9/lab9_2.stan"), chain = 3, iter = 500, refresh = 0)
summary(mod2)[["summary"]][c(paste0("alpha[",1:length(observe.i), "]"), "beta"),]
```

**Model 3:** $\log \theta_i = \alpha_i + \beta x_i$ with $\alpha_i \ + \beta x_i$

```{r echo=FALSE}
stan_df3 <- list(y = observe.i, log_e = log(expect.i), N = length(observe.i), x = aff.i - mean(aff.i))
mod3 <- stan(data = stan_df3, file = here("C:/Users/pquyn/OneDrive/Personal Documents/GitHub/STA2201-W2023/Lab9/lab9_3.stan"), chain = 5, iter = 1000, refresh = 0)
summary(mod3)[["summary"]][c(paste0("alpha[",1:length(observe.i), "]"), "beta", "mu", "sigma"),]
```

## Question 3: Make two plots (appropriately labeled and described) that illustrate the differences in estimated $\theta_i$'s across regions and the differences in $\theta$s across models.
...
```{r echo=FALSE}
ggp1 <- res |> select(median_mod1, median_mod2, median_mod3)|>
...
```{r echo=FALSE}
ggp2 <- res|> mutate(deaths = observe.i)|>
              ...
              ggplot(aes(log_smr,median_mod1, color = "Model 1")) + 
                     ...
                     geom_errorbar(aes(ymin = lower_mod2, ymax = upper_mod2, color = 'Model 2'))+
...
```{r echo=FALSE}
ggarrange(ggp1, ggp2, nrow = 1, ncol = 2, common.legend = FALSE) 
```