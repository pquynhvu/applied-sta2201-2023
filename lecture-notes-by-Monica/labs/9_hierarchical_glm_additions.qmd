...
1. Intercept $\alpha_i$ is same in each region $= \alpha$ (with covariate)
...
Look at three models for $\log \theta_i$:
...
mod1 <- stan(data = stan_data, file = "../code/models/lab9_1.stan")
mod1
```
...
```{r}
mod2 <- stan(data = stan_data, file = "../code/models/lab9_2.stan")
mod2
```
...
```{r}
mod3 <- stan(data = stan_data, file = "../code/models/lab9_3.stan")
mod3
```
