...
```{r}
kidiq %>% 
  mutate(mom_high_school = ifelse(mom_hs==1, "at least high school", "less than high school")) %>% 
  ggplot(aes(kid_score, fill = mom_high_school)) + 
  geom_histogram(position = "dodge") + 
  scale_fill_brewer(palette = "Set1", name = "Mother's education") + 
  xlab("Kid's test score")
```

```{r}
kidiq %>% 
  mutate(mom_high_school = ifelse(mom_hs==1, "at least high school", "less than high school")) %>% 
  ggplot(aes(x = mom_iq, y = kid_score, color = mom_high_school)) + 
  geom_point() + 
  scale_color_brewer(palette = "Set1", name = "Mother's education") + 
  geom_smooth(method = "lm")+
  labs(y = "Kid's test score", x = "Mother's IQ")
```
...
Pretty correlated, potentially a problem for convergence
...
fit2 %>% 
  ...
  group_by(.draw) %>%
  mutate(.value = ifelse(!is.na(condition)&condition==2, .value*10,  .value)) %>% 
...
```{r}
samps <- extract(fit2)
linpred <- samps[["alpha"]] + samps[["beta"]][,1] + samps[["beta"]][,2]*-5
sigmas <- samps[["sigma"]]
ypred <- tibble(ypred = rnorm(length(sigmas), mean = linpred, sd = sigmas))
ggplot(ypred,aes(ypred)) + geom_histogram(fill = "firebrick4", col = "navy") + ggtitle("Distribution of predicted scores for new kid with mom's IQ = 95")
```
